...
class DbManager:
    ...
    def _init_schema(self) -> None:
        ...
        with self._connect() as conn:
            ...
            if "avg_snr" not in columns:
                conn.execute("ALTER TABLE all_passes ADD COLUMN avg_snr REAL")
            if "sum_snr" not in columns:
                conn.execute("ALTER TABLE all_passes ADD COLUMN sum_snr REAL")
        if self.logger:
        ...
        with self._connect() as conn:
            cur = conn.execute(
                ...
                INSERT INTO all_passes (
                    station_name, satellite_name, pass_date,
                    pass_start_time, pass_end_time,
                    rx_start_time, rx_end_time,
                    avg_snr, sum_snr, log_url, log_path, graph_url, graph_path,
                    success
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
    ...
    def get_daily_station_stats(self, stat_day: date | datetime | str) -> list[list]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                SELECT s.station_name,
                       ...
                       s.failed_passes,
                       s.failed_percent,
                       ROUND(AVG(p.avg_snr), 2) AS avg_snr
        ...
        result = []
        for station_name, total, success, failed, failed_percent, avg_snr in rows:
            result.append([station_name, total, success, failed, failed_percent, avg_snr or 0.0])
        return result

    def get_max_sum_snr_passes(self, stat_day: date | datetime | str) -> list[SatPas]:
        ...
        with self._connect() as conn:
            ...
            rows = conn.execute(
                ...
                FROM all_passes p
                JOIN (
                    SELECT station_name, MAX(sum_snr) AS max_sum_snr
                    FROM all_passes
                    WHERE pass_date = ?
                      AND sum_snr IS NOT NULL
                    GROUP BY station_name
                ) mx
                  ON p.station_name = mx.station_name
                 AND p.sum_snr = mx.max_sum_snr
                WHERE p.pass_date = ?
...
if __name__ == "__main__":
    ...
    assert row is not None, "stats row not found"
    total_passes, success_passes, failed_passes, failed_percent = row
    assert total_passes == 2, "total_passes should be 2"
    assert success_passes == 1, "success_passes should be 1"
    assert failed_passes == 1, "failed_passes should be 1"
    assert round(float(failed_percent), 2) == 50.0, "failed_percent should be 50"
    assert commercial_id > 0, "commercial pass insert failed"

    print("DbManager tests passed")
