...
class SatPas:
    ...
    Класс хранит метаданные пролета, временные метки и ссылки на артефакты
    (лог/график). Все поля опциональны и могут заполняться по мере получения
    данных.
    ...
    Атрибуты:
        ...
        location: Местоположение станции (строка).
        pass_date: Дата пролета.
        pass_start_time: Время начала пролета.
        pass_end_time: Время окончания пролета.
        rx_start_time: Время начала приема.
        rx_end_time: Время окончания приема.
        awg_snr: Средний SNR.
        max_snr: Максимальный SNR.
        sum_snr: Сумма SNR.
        log_url: URL лога.
        log_path: Локальный путь к логу.
        graph_url: URL графика.
        graph_path: Локальный путь к графику.
        success: Признак успешного пролета.
    """
    ...
    def __init__(
        ...
        """Создает запись пролета спутника.

        Аргументы соответствуют одноименным атрибутам класса и допускают
        поэтапное заполнение, поэтому по умолчанию большинство значений None.
        """
